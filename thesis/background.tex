...
\begin{lemma}
	Let $\splx^\du$ be the dual of the simplex $\splx\in\R^{n-1}$ with $\Sv(\splx)=(\sv_1,\dots,\sv_n)$ and $\Sv(\splx^\du)=(\bgamma_1,\dots,\bgamma_n)$. Then $\bgamma_i$ is perpendicular to $\splx_1[\ic]$ for all $i\in[n]$. 
\end{lemma}
\begin{proof}
Consider a fixed $i<n$. Let $\p,\q\in\splx_\ic$ have barycentric coordinates $\x$ and $\y$ respectively. We need to show that $\la \bgamma_i,\p-\q\ra =0$. Note that $x(i)=y(i)=0$, and so 
\begin{align*}
...
as desired. We now consider $i=n$. Recall that $\bgamma_n=-\sum_{i<n}\bgamma_i$. Moreover, $\la \bgamma_i,\sv_j\ra = \delta_{i,j}-\la \bgamma_i,\sv_n\ra$. Using similar arithmetic as above, 
\begin{align*}
