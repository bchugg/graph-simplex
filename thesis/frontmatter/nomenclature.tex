...
\begin{longtable}{p{\colwidth}p{\descsep}l}
	...
	$\{\bgam_i\}$ & Vertices of $\ssplx$ & \\
	$\cong$ & Translational congruency between simplices & \\
	$\x_{U}$ & Barycentric coordinate for face $\ssplx_U$ & \\
	...
	$\ssplx_0$ & Canonical/Centred Simplex of $\ssplx$ & Definition \ref{def:centred_simplex} \\
	$[\ssplx]$ & Congruence class of simplices & Equation \eqref{eq:[ssplx]}\\

	$\ssplx^\du$ & Dual simplex to $\ssplx$ & Section \ref{sec:background_dual_simplex} \\
	$\Sv^\du=\Sv(\ssplx^\du)$ & Dual vertex matrix\\
	$\{\bgam^\du_i\}$ & Vertices of $\ssplx^\du$ & \\
		

	$\splx_G$ ($\splxn_G$)  &  (Normalized) Simplex of $G$ &  Section \ref{sec:bijection_graphs_simplices} \\
	...
	$\theta_{ij}$ ($\theta_{ij}^+)$ & Angle between $\splx_\ic$ ($\splx_\ic^+$) and $\splx_\jc$  ($\splx_\jc^+$) &  \\
	$\phi_{ij}$ ($\phi^+_{ij})$ & Angle between $\sv_i$ ($\sv_i^+)$ and $\sv_j$ ($\sv_j^+)$ &  \\
	
	$\H_i$ ($\H_i^+$) & Hyperplane containing $\splx_\ic$ ($\splx_\ic^+$) & Equation~\eqref{eq:Hi}\\
	...
	$\bxi$ & Avg. squared distance of vertices minus $\bar{d}$ & Equation \eqref{eq:avg_sqd_distance_i}\\	
\end{longtable}
\vspace{\groupsep}
...
\vspace{\headingsep} 
\begin{longtable}{p{\colwidth}p{\descsep}l}
	$\norm{\cdot}_p$ & $p$-norm in $\R^d$ & Section~\ref{sec:background_linear}\\
	$\perp$ & Perpendicular to; orthogonal complement &\\
	$\dim \Q$ & Dimension of space spanned by $\Q$ \\
	$\range\ Q$ & Range of $\Q$ \\
	$\ker\Q$ & Kernel of $\Q$\\
	...
	$\{\lambda_i\}$ ($\{\lambdan_i\}$) & Eigenvalues of $\L_G$ ($\Ln_G$) & Section \ref{sec:background_laplacian}\\
	$\Eval_G$ ($\Evaln_G$) & Diagonal Eigenvalue matrix of $\L_G$ ($\Ln_G$) \\
	$\{\vp_i\}$ ($\{\vpn_i\}$) & Eigenvectors of $\L_G$ ($\Ln_G$)\\
	{$\Eig_G$ ($\Eign_G$)} & Eigenvector matrix of $\L_G$ ($\Ln_G$)\\
	
	$\bD$ & Vector of diagonals of $\L_G^+$ & Section~\ref{sec:background_er}\\
